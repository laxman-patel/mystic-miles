...
export default function Form({dataRes}) {
  ...
  <div className="fixed py-16 flex flex-col justify-center lg:justify-between lg:block top-[18vh] lg:top-[3vh] w-full h-auto lg:top- mx-10 lg:w-2/3 z-50 bg-white  md:px-16 md:py-16 lg:px-28 lg:py-16 ">
         <div className="flex justify-center lg:justify-between items-center">
            <div>
                <h1 className="font-serif text-[12vw] lg:text-9xl text-center lg:text-start font-semibold">Get in touch</h1>
        ...
        <div className="flex items-center justify-between mt-24 lg:mt-0">
            ...
            <form onSubmit={onSubmit} method="POST" className=" w-full flex flex-col items-center justify-between lg:pb-10 lg:pl-6">
                <div className="flex flex-col justify-center font-sans lg:py-20 items-center">
                  <input name="Full name" className="bg-white text-[4vw] lg:text-3xl border-b lg:border-b-2 border-black focus:outline-none placeholder-black/60" value={name} onChange={(event) => {setName(event.target.value)}} type="text" placeholder="Full name" />
                  <input name="Contact Number" className="bg-white pt-14 lg:pt-24 border-b lg:border-b-2 text-[4vw] lg:text-3xl border-black focus:outline-none placeholder-black/60"  value={contact} onChange={(event) => {setContact(event.target.value)}} type="tel" placeholder="Contact number" />
                  ...
                  <select  onChange={(event) => {setModel(event.target.value)}} className="bg-white mt-14 lg:mt-24 border-b lg:border-b-2 w-full text-[4vw] lg:text-3xl border-black focus:outline-none placeholder-black/60" name="Model Name">
                   <option key={0}>Choose Model</option>
                    {dataRes.map((item) => <option key={dataRes.indexOf(item) + 1} value={item}>{item}</option>)}
                  </select>
                </div>
                <button disabled={submitting} type="submit" className="bg-red mt-20 hover:bg-black transition-all flex items-center justify-between lg:w-1/3 tracking-wider hover:cursor-pointer font-sans text-[3vw] lg:text-xl text-white font-light px-4 py-2 lg:px-8 lg:py-4">
